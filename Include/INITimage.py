...
def get_file(file_dir):
    ...
    for file in os.listdir(file_dir + '/玉石'):
        yushi.append(file_dir + '/玉石' + '/' + file)
        label_yushi.append(0)
    for file in os.listdir(file_dir + '/瓷器'):
        ciqi.append(file_dir + '/瓷器' + '/' + file)
        label_ciqi.append(1)

    # 打印出提取图片的情况，检测是否正确提取
    # print("有 %d 张 玉石\n有 %d 张 瓷器\n有 %d 张 邮票" % (
    # len(yushi), len(ciqi), len(youpiao)), end="")
    print("有 %d 张 玉石\n有 %d 张 瓷器\n" % (
    len(yushi), len(ciqi)), end="")
    ...
    image_list = np.hstack((yushi, ciqi))
    label_list = np.hstack((label_yushi, label_ciqi))
    # 利用shuffle，转置、随机打乱
...
def get_batch(image, label, image_W, image_H, batch_size, capacity):
    ...
    # print(label_batch) Tensor("Reshape:0", shape=(2,), dtype=int32)
